...
import { updateMediaResource, uploadPeFile } from "./peMediaApi.js";
...
export default function AudioEditModal({ resource, onClose, onSaved }) {
  ...
  const submit = async () => {
    if (!title.trim()) return alert("곡 제목을 입력하세요");
    setSaving(true);
    try {
      const patch = {
        title: title.trim(),
      ...
      if (coverFile) {
        ...
        const coverPath = `audio/covers/${Date.now()}_${Math.random().toString(36).slice(2, 8)}.${coverExt}`;
        patch.cover_url = await uploadPeFile(coverPath, coverFile);
      }
      await updateMediaResource(resource.id, patch);
      onSaved();
      onClose();
    } catch (e) {
  ...
  return (
    <ModalShell title="음원 수정" onClose={onClose}>
      <label style={peLbl}>곡 제목 *</label>
      ...
      <label style={peLbl}>앨범 아트 변경 (선택)</label>
      <input type="file" accept="image/*" onChange={e => setCoverFile(e.target.files?.[0] || null)} style={{ ...peInp, padding: 10 }}/>
